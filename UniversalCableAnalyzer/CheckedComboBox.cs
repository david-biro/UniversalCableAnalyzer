...
namespace CheckComboBoxTest {
    public class CheckedComboBox : ComboBox {
        ...
        /// </summary>
        /// 
        private bool[] tempCheckedState;
        private object[] tempObjCollection;
        internal class Dropdown : Form {
            ...
            internal class CustomCheckedListBox : CheckedListBox {
                ...
                protected override void OnMouseMove(MouseEventArgs e) {
                    ...
                    if (index == 0) {       // some tweak to uncheck the rest of the list if user chooses "0", the first item in the list
        ...
        private void DoDropDown()
        {
            if (!dropdown.Visible)
            {
                if (dropdown == null || dropdown.IsDisposed)
                {
                    ...
                    dropdown.List.Items.AddRange(tempObjCollection);
                    //for (int i = 0; i < COUNT; i++)
                    //    SetItemChecked(i, itemCheckedStatus[i]);
                    for (int i = 0; i < tempCheckedState.Length; i++)
                        SetItemChecked(i, tempCheckedState[i]);
        ...
        protected override void OnDropDownClosed(EventArgs e)
        {
            ...
            {
                tempObjCollection = new object[Items.Count];

                Items.CopyTo(tempObjCollection, 0);

                tempCheckedState = new bool[dropdown.List.Items.Count];

                for (int i = 0; i < dropdown.List.Items.Count; i++)
                    tempCheckedState[i] = GetItemChecked(i);
